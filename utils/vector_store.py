...
from utils.constants import EMBEDDING_MODEL, CHUNK_SIZE, CHUNK_OVERLAP, VECTORSTORE_DIR, EMBEDDING_BATCH_SIZE
...
def create_vector_store(documents, store_id):
    ...
    persist_dir = os.path.join(VECTORSTORE_DIR, store_id)
    ...
    print(f"Successfully created vector store for {store_id}")
    return db


def load_vector_store(store_id):
    embeddings = get_embeddings()
    persist_dir = os.path.join(VECTORSTORE_DIR, store_id)
...
def check_vector_store_exists(store_id):
    persist_dir = os.path.join(VECTORSTORE_DIR, store_id)
